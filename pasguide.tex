...
\end{equation}
The cable shape is approximated by the polygonal chain connecting consecutive nodes. The node ordering is part of the configuration used for transition validation; it is not only a drawing convention for an unlabelled curve. The \(k\)-th segment is
\begin{equation}
s_k(C)
=
[p_k,p_{k+1}],
...
blocked one, emphasizing that obstacle avoidance is checked for the continuous
local deformation, not only at the two endpoint configurations.
...
=
1 .
\label{eq:inserted_edge_swept_clear_valid}
\end{equation}
